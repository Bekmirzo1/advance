...
// *transition-dela more on why__title
window.addEventListener('load', function () {
    const whyTitle = document.querySelector('.why__title');
    if (whyTitle) {
        const fullTitle = document.querySelector('.fullscreen__title');
        const conditionWhy = window.innerHeight > whyTitle.getBoundingClientRect().top && whyTitle.getBoundingClientRect().top > 0;
        const conditionFull = window.innerHeight > fullTitle.getBoundingClientRect().bottom && fullTitle.getBoundingClientRect().top > 0;
        if (conditionWhy && conditionFull) {
            whyTitle.classList.add('_trs-delay')
        }
    }
});
//BildSlider
...
if (goto_links) {
	for (let index = 0; index < goto_links.length; index++) {
		...
		goto_link.addEventListener('click', function (e) {
            ...
            menu_close()
			_goto(target_block, 800, document.querySelector('.header__wrapper').offsetHeight + 50);
			e.preventDefault();
		});
	}
}
